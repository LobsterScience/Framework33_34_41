...
g = subset(g, SYEAR<=2026 & LFA==33)
po = lobster.db('port_location')
#bring in voluntary log data to populate <2005
v = lobster.db('process.vlog')
v = subset(v,LFA %in% c('33' ))

#assign port code to most common grid
vPC = unique(v$PORT_CODE)
gPC = aggregate(SD_LOG_ID~GRID_NUM+LFA+COMMUNITY_CODE, data=subset(g, LFA==33 & SYEAR<=2020 & COMMUNITY_CODE %in% vPC),FUN=length)
library(dplyr)
...
aT = merge(aT,d,by.x=c('LFA','yr','DOY','GRID_NUM'),by.y=c('LFA','yr','doy','GRID_NO'),all.x=T)
#####
saveRDS(aT, 'ExtendedTS_LFA33_logs.rds')
