...
te = readRDS(file=file.path(project.datadirectory('bio.lobster.glorys'),'Glorys2000-2023wBianCorrColum_LFAGrids_doy.rds'))


##the bias correction model was generated on diff = TEMP - Glor, so neg means glor is high, 
te$GlorBC = te$Glor+te$pred

tea = aggregate(GlorBC~doy+Date+LFA.x+GRID_NO+YR+woy,data=te,FUN=function(x) c(mean(x),sd(x),quantile(x,c(0.25,0.5,0.75)), length(x)))
tea_a = data.frame(doy=tea$doy,
                  Date = tea$Date,
                  LFA = tea$LFA.x,
                  Grid = tea$GRID_NO,
                  YR = tea$YR,
                  woy = tea$woy,
                  GlorBC_mean = tea$GlorBC[,1],
                  GlorBC_sd = tea$GlorBC[,2],
                  GlorBC_q1 = tea$GlorBC[,3],
                  GlorBC_q2 = tea$GlorBC[,4],
                  GlorBC_q3 = tea$GlorBC[,5],
                  GlorBC_n = tea$GlorBC[,6])
                
tea_a$Date = as.Date(tea_a$Date)

saveRDS(tea_a, file=file.path(project.datadirectory('bio.lobster.glorys'),'Glorys2000-2023wBias_LFAGrids_agg_doy.rds'))
...
d = readRDS(file=file.path(project.datadirectory('bio.lobster.glorys'),'Glorys2000-2023wBias_LFAGrids_agg_doy.rds'))

d = subset(d,select=c(LFA,Grid,YR,doy,GlorBC_mean))
cwga$yr = lubridate::year(cwga$DATE_FISHED)
cda = merge(cwga,d,by.x=c('LFA','GRID_NO','yr','DOY'), by.y=c('LFA','Grid','YR','doy'))
...
saveRDS(list(gto,ns_coast,ca),file='data_33_34_41_sdmtmb.rds')
require(mgcv)
require(bio.lobster)
require(spdep)
require(sf)
...
require(sdmTMB)
m4 = sdmTMB(WEIGHT_KG~ s(GlorBC_mean)+s(DOS),
            offset = 'leffort',
            data=ca,
