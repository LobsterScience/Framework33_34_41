...
crp = c(xmin = -70, ymin = 40.75, xmax = -57, ymax = 47.5)
...
gs$mon = lubridate::month(gs$DATE)
gs = subset(gs, mon %in% 6:11)
...
hist(gs$DYEAR)
i = which(gs$SOURCE=='Snow crab survey')
gs$Legal[i] = gs$Lobster[i]

i = which(is.na(gs$Recruit))
gs$Recruit[i] = 0


gs$PreF_C = ifelse(gs$mon<9,gs$Legal+gs$Recruit,gs$Legal)  
gsr = subset(gs,select=c(DATE,YEAR,OFFSET, SOURCE, PreF_C, X,Y, z, Gear))
  
saveRDS(list(gsr,rL,ns_coast),'SurveyOnlyPreFishery_Data.rds')
